...
def triangular(poligono): 
    ...
    for i in range(n):
        indice[poligono[i]] = i
        triangulo = crear_triangulo(i, (i+1)%n, (i+2)%n)   #cada triangulo esta formado por los indices de los vertices en poligono
        triangulaciones[0][(triangulo, )] = {"peso": calcular_peso(triangulo, poligono), "indices_subpoligono": {k for k in range(n) if k != (i+1)%n}}
        ...
        for triangulos, info in triangulaciones[i-1].items():
            ...
            for j in range(m):
                ...
                if nuevo_triangulo not in triangulos:
                    triangulaciones[i][tuple(sorted(list(triangulos) + [nuevo_triangulo]))] = {"peso" : peso + calcular_peso(nuevo_triangulo, poligono), "indices_subpoligono" : indices_subpoligono - {indice[subpoligono[(j+1)%m]]}}
