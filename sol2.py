...
def peso_poligono(
        triangulaciones, poligono, poligono_inicio, sub_triang, poligon_len):
    # el peso del poligono es el peso del triangulo
    # mas el peso de los poligonos que quedan
    # entre el punto y los extremos
    peso_triangulo = calcular_peso(
        poligono[poligono_inicio],
        poligono[sub_triang],
        poligono[poligono_inicio + poligon_len],
    )
...
def triangular(poligono):
    ...
    for poligon_len in range(2, n):
        # itero empezando por los poligonos de menor tamaño
        # a los de mayor tamaño
        for poligono_inicio in range(n - poligon_len):
            # por cada punto del poligono genero el poligono
            # de tamaño poligon_len trazando
            # una linea entre el punto y el punto
            # y poligon_len posiciones mas adelante
            triangulaciones[poligono_inicio][(poligono_inicio + poligon_len)] \
                = min(
                    [
                        ...
                        for sub_triang in range(
                            poligono_inicio + 1, poligono_inicio + poligon_len
                        )
                    ]
                )
            # dentro de ese poligono itero
            # por cada uno de sus puntos que no sean extremos y
            # formo un triangulo entre ese punto y
            # los extremos(poligono_inicio+1,poligono_inicio+poligon_len)

    return triangulaciones[0][
        n - 1
    ]  # este valor representa el peso del poligono completo
